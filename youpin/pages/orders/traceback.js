...
Page({
    ...
    formSubmit: function(e) {
        ...
        if (data.traceback_name.length && data.traceback_no.length) {

            let params = {
                id: this.data.order_id,
                traceback_name: data.traceback_name,
                traceback_no: data.traceback_no
            }

            app.putData('/user/order/traceback', params, (res) => {
                app.showMsg('操作完成');
                wx.navigateBack({
                    delta: 1,
                })
            ...
            let errmsg = '';
            if (!data.traceback_name.length) {
                errmsg += '\r\n请填写快递公司名称';
            }
            if (!data.traceback_no.length) {
                errmsg += '\r\n请填写快递单号';
            }
            app.showMsg('提交失败' + errmsg);
        }
